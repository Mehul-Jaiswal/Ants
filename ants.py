...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if len(self.place.bees) != 0 and self.chew_countdown == 0:
            self.chew_countdown = self.chew_duration+1
        ...
        if self.status:
            ...
            while temp is not None:
                if temp.ant is not None:
                    if temp.ant not in QueenAnt.buffedants:
                        Ant.buff(temp.ant)
                        QueenAnt.buffedants.append(temp.ant)
                    if temp.ant.is_container:   
                        if temp.ant.ant_contained is not None and temp.ant.ant_contained not in QueenAnt.buffedants:
                            Ant.buff(temp.ant.ant_contained)
                            QueenAnt.buffedants.append(temp.ant.ant_contained)
                temp = temp.exit
        else:
